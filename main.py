...
from authentication import oauth2_scheme, get_current_user, create_access_token
from user import User, session, University, College, Course, Student, Semester, Enrollment, Marks
import uvicorn
from fastapi.security import OAuth2PasswordRequestForm
from pydantic_form_details import UniversityCreate, CollegeCreate, CourseCreate, StudentCreate, SemesterCreate, MarkCreate, EnrollmentCreate
...
async def Create_Course(form_data: CourseCreate):
    college = session.query(College).filter(College.name == form_data.college_name).first()
    if college is None:
        raise HTTPException(status_code=404, detail="College not found")
    new_course = Course(name = form_data.name, college_id = college.id)
    new_course.colleges.append(college)
...
async def Find_CourseTopper_Semester(course_id: int = Query(..., description = 'Course ID'), semester_id: int = Query(..., description = 'Semester ID')):
    ...
    student = session.query(Student).get(mark.student_id)
    return student.name
...
async def Find_UniTopper_Semester(semester_id: int = Query(..., description = 'Semester ID'),uni_id: int = Query(..., description = 'University ID')):
    colleges = session.query(College).filter(College.uni_id == uni_id).all()
    college_ids = [college.id for college in colleges]
    courses = session.query(Course).join(Course.colleges).filter(College.id.in_(college_ids)).distinct().all()
    student_toppers = []
    for course in courses:
        max_mark = session.query(func.max(Marks.mark)).filter(Marks.course_id == course.id, Marks.semester_id == semester_id).scalar()
        if max_mark is None:
            continue
        mark = session.query(Marks).filter(Marks.course_id == course.id, Marks.semester_id == semester_id, Marks.mark == max_mark).one_or_none()
        if mark is None:
            continue
        student = session.query(Student).get(mark.student_id)
        student_toppers.append(student.name)
    series = pd.Series(student_toppers)
...
async def Get_Course():
    ...
    for course in all_course:
        college_names = [college.name for college in course.colleges]
